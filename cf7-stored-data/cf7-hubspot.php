...
self::$plugin->instance();
 ...
 public function form_submitted($form){ 
    ...
    if(class_exists('vxcf_form')){ return; }
		...
		if ($lead)
		{
			foreach ($lead as $fieldName => $filepath)
			{

			  // Check the type of file. We'll use this as the 'post_mime_type'.
			  $filetype = wp_check_filetype(basename($filepath), null);

			  $attachFileName = $wp_upload_dir['path'] . '/' . basename($filepath);

			  $attachFile_Url = $wp_upload_dir['url'] . '/' . basename($filepath);

			  copy($filepath, $attachFileName);

			}

		  $lead = array("myimgfile"=>$attachFile_Url); // new url as per update by bytes
		}
